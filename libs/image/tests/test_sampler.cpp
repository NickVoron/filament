...
// Writes a 1-channel or 3-channel image out to disk.
static void saveImage(const image::LinearImage& limg, const utils::Path& fname);
...
TEST_F(ImageTest, LuminanceFilters) { // NOLINT
    ...
    saveImage(image::hstack({grays0, grays1}), "grays.png");
}
...
static void saveImage(const image::LinearImage& limg, const utils::Path& fname) {
    ...
    ImageEncoder::encode(out, ImageEncoder::Format::PNG, img, "", fname);
}
